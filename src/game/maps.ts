...
function buildRapis(): string[] {
  const g = grid(34, 26, ',')
  frame(g, '#')
  // ── ゾーニング(パッケージC): 北=土路地の住宅区。中央の縦通り帯(x15-19)は空けておく ──
  fill(g, 2, 2, 14, 6, 'D')
  fill(g, 20, 2, 32, 6, 'D')
  ...
  // ── ゾーニング(パッケージC): 中央=石畳広場(噴水+転送門を含む一帯) ──
  fill(g, 10, 13, 24, 21, 'P')
  fill(g, 17, 1, 17, 24, '.') // 縦の大通りは広場より優先して通す(再度上書き)
  fill(g, 2, 13, 31, 13, '.') // 横の大通りも同様
  fill(g, 20, 15, 22, 16, 'H') // 道具屋(広場ゾーン内なので再度確保)
  // ── 崖のテラス(パッケージC): 北東の空き地に一段高い広場を追加。四方を崖で囲み、
  //    出入口は南辺の階段(往復可)とレッジ(南へ飛び降りる一方通行)のみに限定する。
  //    西端(x23,y3-4)と東端(x29,y3-4)は民家C/Dの建物(H)がすでに壁として機能するのでCで上書きしない。
  fill(g, 24, 2, 28, 4, 'D') // テラス床
  fill(g, 23, 1, 29, 1, 'C') // 北壁
  set(g, 23, 2, 'C'); set(g, 23, 5, 'C') // 西壁(y3-4は民家Cの建物で代用)
  set(g, 29, 2, 'C'); set(g, 29, 5, 'C') // 東壁(y3-4は民家Dの建物で代用)
  fill(g, 24, 5, 28, 5, 'C') // 南壁(崖の縁)
  set(g, 26, 5, 'S') // 階段(南壁を貫く・両面通行の登り口)
  set(g, 24, 5, 'L') // レッジ(テラス側からのみ南へ降りられる。北へは登れない)
  // 花壇の装飾
...
export const MAPS: Record<string, GameMap> = {
  rapis: {
    ...
    npcs: [
      ...
      // ── 遊び心: 看板猫・番犬・かくれんぼっ子 ──
      { x: 15, y: 12, kind: 'villager', name: '看板猫ミケ', emoji: '🐈', sprite: 'npc_cat', lines: ['ニャ〜ン。（撫でてほしそうに すり寄ってきた）', '（喉を ゴロゴロ鳴らしている。なんだか 幸運が 舞い込みそうだ。）'] },
      { x: 28, y: 12, kind: 'villager', name: '番犬ロッキー', emoji: '🐕', sprite: 'npc_dog', lines: ['ワンッ！ ……君は 悪いやつじゃ なさそうだ、と 尻尾を 振っている。', 'グルル……灰化した幻獣の匂いには、めっぽう 敏感らしい。'] },
      { x: 31, y: 5, kind: 'villager', name: '？？？', emoji: '🧒', sprite: 'npc_kid', lines: ['しーっ！ いま かくれんぼ中なんだ。', '鬼に 見つかっちゃうから……あっち 行ってて！'] },
    ],
    buildings: [
    ...
    props: [
      ...
      // ── 崖のテラス(パッケージC): 階段口の目印＋見晴らしの装飾 ──
      { x: 25, y: 5, kind: 'lamp', solid: true }, { x: 27, y: 5, kind: 'lamp', solid: true }, // 階段(26,5)を挟む灯り
      { x: 26, y: 2, kind: 'sign', name: '見晴らし台', lines: ['村を見下ろせる、ちょっとした高台。', '西の階段からも、北西の崖から飛び降りても 行き来できる。'] },
      { x: 25, y: 3, kind: 'flower' }, { x: 27, y: 4, kind: 'flower' },
    ],
    chests: [
      { x: 26, y: 3, id: 'rapis_terrace', item: 'flask', amount: 1 }, // 崖のテラス(パッケージC)
...
export function isLedgeBlocked(ch: string, dy: number): boolean {
  return ch === 'L' && dy < 0
}
